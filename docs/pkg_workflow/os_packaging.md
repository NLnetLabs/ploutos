...
The Pluotos workflow can package your Rust Cargo application as one or both of the following common Linux O/S package formats:
...
The `pkg` and `pkg-test` jobs of the Pluotos workflow package your Rust Cargo application into one or more of these formats, run some sanity checks on them and verify that they can be installed, uninstalled, and (optionally) upgraded, plus (if configured) can also run tests specific to your application on the installed package.

Binaries to be included in the package are either pre-compiled by the [`cross` job](./cross_compiling.md) of the Pluotos workflow, or compiled during the `pkg` job.
...
Inputs to the Pluotos workflow common to all package types are:

TODO

Inputs to the Pluotos workflow specific to DEB packaging are:

TODO

Inputs to the Pluotos workflow specific to RPM packaging are:
